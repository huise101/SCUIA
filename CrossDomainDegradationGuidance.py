...
class CrossDomainDegradationGuidance(nn.Module):
    ...
    5. Underwater quality proxy terms for color and contrast robustness
    6. MLP projection g(·) for embedding alignment
    7. Wasserstein distance with high-quality reference set S_ref
    
    Args:
        p: Reference GT features for comparison
    ...
    def __init__(self, p=None, input_dim=1500, stability_scale=0.001, args=None):
        ...
        self.ref_color_balance = None
        self.ref_visibility = None
        self.ref_perceptual = None
        ...
        if p is not None:
            self.compute_gt_reference(p.unsqueeze(0))
        else:
    ...
    def forward(self, x):
        ...
        color_balance_proxy, visibility_proxy, perceptual_proxy = self.compute_hydro_quality_proxies(x)
        ...
        # Add underwater proxy alignment penalties relative to GT reference.
        # We bound each term with tanh to avoid unstable score jumps.
        color_balance_gap = torch.tanh(torch.abs(color_balance_proxy - self.ref_color_balance))
        visibility_gap = torch.tanh(torch.abs(visibility_proxy - self.ref_visibility))
        perceptual_gap = torch.tanh(torch.abs(perceptual_proxy - self.ref_perceptual))
        proxy_alignment = (
            0.20 * color_balance_gap
            + 0.35 * visibility_gap
            + 0.45 * perceptual_gap
        )
        ...
        final_score = base_score * (
            1
            + 0.88 * color_feature.mean()
            + 0.05 * contrast_feature.mean()
            + 0.75 * proxy_alignment.mean()
        )

        return final_score.squeeze()

    def compute_gt_reference(self, p):
        """Compute reference statistics from high-quality GT images (S_ref)."""
        # Dimension matching
        ...
        ref_color_balance, ref_visibility, ref_perceptual = self.compute_hydro_quality_proxies(p)
        self.ref_color_balance = ref_color_balance.mean(dim=0, keepdim=True).detach()
        self.ref_visibility = ref_visibility.mean(dim=0, keepdim=True).detach()
        self.ref_perceptual = ref_perceptual.mean(dim=0, keepdim=True).detach()
    ...
    def _feature_rgb_proxy(self, x):
        """Build pseudo RGB channels from feature tensors for underwater metric proxies."""
        if x.size(-1) >= 3:
            rgb = x[..., :3]
        elif x.size(-1) == 2:
            rgb = torch.cat([x, x[..., :1]], dim=-1)
        else:
            rgb = x[..., :1].repeat(1, 1, 3)
        return torch.sigmoid(rgb)

    def compute_hydro_quality_proxies(self, x):
        """Compute lightweight underwater quality proxies on latent features."""
        eps = 1e-6
        rgb = self._feature_rgb_proxy(x)
        r, g, b = rgb[..., 0], rgb[..., 1], rgb[..., 2]

        rg = r - g
        yb = 0.5 * (r + g) - b
        mean_rg = torch.mean(rg, dim=-1, keepdim=True)
        mean_yb = torch.mean(yb, dim=-1, keepdim=True)
        std_rg = torch.std(rg, dim=-1, keepdim=True, unbiased=False)
        std_yb = torch.std(yb, dim=-1, keepdim=True, unbiased=False)
        color_balance_proxy = torch.sqrt(mean_rg ** 2 + mean_yb ** 2 + eps) + torch.sqrt(std_rg ** 2 + std_yb ** 2 + eps)

        chroma = torch.sqrt(rg ** 2 + yb ** 2 + eps)
        luminance = 0.299 * r + 0.587 * g + 0.114 * b
        saturation = chroma / torch.sqrt(chroma ** 2 + luminance ** 2 + eps)
        avg_saturation = torch.mean(saturation, dim=-1, keepdim=True)
        chroma_std = torch.std(chroma, dim=-1, keepdim=True, unbiased=False)
        lum_q01 = torch.quantile(luminance, 0.01, dim=-1, keepdim=True)
        lum_q99 = torch.quantile(luminance, 0.99, dim=-1, keepdim=True)
        lum_contrast = (lum_q99 - lum_q01).clamp_min(0.0)
        visibility_proxy = 0.4680 * chroma_std + 0.2745 * lum_contrast + 0.2576 * avg_saturation

        lum_max = torch.amax(luminance, dim=-1, keepdim=True)
        lum_min = torch.clamp(torch.amin(luminance, dim=-1, keepdim=True), min=0.05)
        local_contrast_proxy = torch.log((lum_max + eps) / (lum_min + eps))
        perceptual_proxy = 0.0282 * color_balance_proxy + 0.2953 * lum_contrast + 3.5753 * local_contrast_proxy

        return color_balance_proxy, visibility_proxy, perceptual_proxy
...
def compute_degradation_guidance_distance(model, test_dataset, img_dir, data_loc, config, sample_callback=None):
    ...
    with torch.no_grad():
        ...
        # Extract reference features from GT images
        first_patches = gt_reference(config).to(config.device)
        ...
        for batch, (x, y, name) in enumerate(tqdm(loader)):
            ...
            if scaled_score.shape == torch.Size([]):
                ...
                sample_scores = scaled_score.cpu().detach().tolist()
            sample_mos = y.tolist()
            sample_names = list(name)
...
def build_degradation_guidance_module(model, config):
    first_patches = gt_reference(config).to(config.device)
    all_ref_feats = model_features(model, first_patches)
    return CrossDomainDegradationGuidance(all_ref_feats).to(config.device)


def compute_degradation_guidance_single_image(
    model, test_image_path, config, tensor_return=False, fd_module=None, clear_cache=True
):
    ...
    with torch.no_grad():
        ...
        if fd_module is None:
            fd_module = build_degradation_guidance_module(model, config)
...
def gt_reference(config):
    """Extract high-quality reference set S_ref for Wasserstein distance computation.
    
    Constructs reference distribution from GT underwater images by selecting
    patches with high sharpness and colorfulness.
    """
    gt_img_dir = config.gt_img_dir
    ps = config.patch_size

    cache_file = 'gt.hdf5'
    cache_file_with_params = 'gt_patches_%03d_%0.2f_%0.2f.hdf5' % (
        config.patch_size, config.sharpness_param, config.colorfulness_param)
    if not os.path.isfile(cache_file) and os.path.isfile(cache_file_with_params):
        cache_file = cache_file_with_params
    ...
    if not os.path.isdir(gt_img_dir):
        raise FileNotFoundError(
            f"GT directory does not exist: '{gt_img_dir}'. Provide --gt_img_dir or place {cache_file} in cwd."
        )

    toten = transforms.ToTensor()
    refs = os.listdir(gt_img_dir)
    if not refs:
        raise ValueError(f"GT directory is empty: '{gt_img_dir}'")

    if not os.path.isfile(cache_file):
        print('Constructing high-quality reference set S_ref (first time initialization)')
        
        # Process first reference image
        temp = np.array(Image.open(os.path.join(gt_img_dir, refs[0])))
        temp = toten(temp)
        ...
        for irx, rs in enumerate(tqdm(refs)):
            temp = np.array(Image.open(os.path.join(gt_img_dir, rs)))
            temp = toten(temp)
            batch = temp.to(config.device).unsqueeze(dim=0)
            patches = batch.unfold(1, 3, 3).unfold(2, ps, ps).unfold(3, ps, ps)
            patches = patches.contiguous().view(1, -1, 3, ps, ps)
